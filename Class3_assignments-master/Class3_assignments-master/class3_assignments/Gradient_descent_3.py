##Using Gradient Descent method with another loss function(np.mean((y-y_hat)**0.5)).
# The things need to do is that change the loss formula and deritive_k and deritive_b.
#There is a bug, and i haven't solve the problem yet,  i'll appreciate it if teacher can point it out
...
def derivative_k(x,y,y_hat):
    # print("type({}), type({})".format(y, y_hat))
    abs_value=[1 if (y_i - y_hat_i) > 0 else -1 for y_i, y_hat_i in zip(y, y_hat)]
    print("{},{}".format(y, y_hat))
    return np.mean([-x_i * abs / (2 * (y_i-y_hat_i)**0.5) for x_i, abs, y_i, y_hat_i in zip(x, abs_value,y,y_hat)])

def derivative_b(y,y_hat):

    abs_value = [1 if (y_i - y_hat_i) > 0 else -1 for y_i, y_hat_i in zip(y, y_hat)]

    return np.mean([-1 * abs / (2 * (y_i - y_hat_i) ** 0.5) for abs, y_i, y_hat_i in zip(abs_value, y, y_hat)])
